...
- Writing `/etc/pacman.d/mirrorlist` needs root. Answering `y` asks for the sudo password first, before the download, so the user is present for it and a refusal costs nothing; the installs use the cached credential and may ask once more if the rate test outlasts sudo's cache. The run must survive a refused or expired password without clearing the flag. (Changed 2026-09-19 after review measured the cost of a late refusal: a repeat of the full download.)
...
- KTD7. **The validation gate tests that ranking happened, not just that a file exists.** The result is rejected unless every uncommented line matches `Server = https://`, there are at least ten `Server` lines, and reflector's verbose output shows at least ten mirrors rated at a non-zero rate. A failed rate returns zero rather than aborting, so on a link that serves the status file but times out most downloads reflector exits zero with a list that is mostly unranked; requiring ten rated keeps the flag armed on such a link until a better one is reached, which is F1's intent. (Raised from one to ten on 2026-09-19, the owner's call after review.) Governs R9, R10.
