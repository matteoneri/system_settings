# Global Development Standards — FNA account

This is the **FNA** codex home (`~/.codex-fna`), selected by the `codex()` wrapper in
`~/.zshrc` when the working directory is under `Projects/ActiveProjects/FNA`, or by
`codex --fna`. The sibling home is `~/.codex-own` (personal account). Both homes can
read and write both project trees; only the account, history and sessions differ.
