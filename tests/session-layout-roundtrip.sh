...
assert_jq nested.json A '[.terminals[] | select(.agent == "codex")][0].config_dir == "/tmp/fixture-codex-home"' "codex terminal carries its CODEX_HOME"
...
if source "$RESTORE" >/dev/null 2>&1; then
    ...
    assert_false valid_config_dir "$HOME/.codex-fna"
    assert_true  valid_codex_home "$HOME/.codex-fna"
    assert_true  valid_codex_home "$HOME/.codex-own"
    assert_false valid_codex_home "$HOME/.claude-fna"
    assert_false valid_codex_home "/etc"
    assert_false valid_codex_home ""
else
